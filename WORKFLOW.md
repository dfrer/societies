...
- The Godot-hosted `Test_GodotCivicDeterministicLoopSmoke` passed for both policy paths. The manifest now declares 481 managed tests and 24 Godot tests. Independent deep review is GO with no P0-P2 findings. Required `build-test-smoke` passed in 4m22s, and PR #173 merged head `a394a8f` as `f8eb222`.
...
- The first F1 Candidate package was rejected by the user as far too realistic. Its three frames remain failed evidence only. Tactile clay-and-wood miniature is now the selected macro language, with Hearthwood Causeway, Reed-Kiln Wetlands, and Painted Sluice Toyworks as the replacement comparison.
- The replacement study is presentation-only: orthographic tabletop staging, matte non-metallic materials, chunky citizen tokens, no cinematic fog, three materially distinct handmade interaction treatments, and seven world-space pointer targets with keyboard-shortcut parity. The optional diagnostic detail panel is hidden by default. Actual-camera regression coverage confirms all seven controls avoid visible Canvas occluders at 1920×1080, 1280×720, and 960×540 across every treatment. It adds no runtime authority, simulation state, cognition, provider, credential, network, or payment action.
- Focused replacement tests pass 14/14. The authoritative wrapper passes 499/499 managed and 26/26 Godot tests, including the physical-control/non-overlap smoke. Release and ExportRelease builds have zero warnings/errors, and the standalone headless study exits cleanly. The wrapper emitted the existing test-intent frame-backlog warning and the known optional runtime-metrics artifact access-denied warning; both associated tests passed. Human visual/play acceptance and study performance remain open.
- Rejected-v1 implementation commit `8820734ecc8afd1a592c9077a9e441c5c4467d5d` and its handoff commits are pushed in [stacked PR #179](https://github.com/dfrer/societies/pull/179), whose base is the open recovery-plan branch/PR #178 at `75b23e9de20be4797a233cb78ff8e8e1b3cd4daf`. Its historical validation is superseded by the separate local replacement evidence above, but the tactile replacement is not yet committed or pushed. GitHub reports no checks because the tests workflow is filtered to PRs based on `master` or `main`; the stacked base is excluded. This is not a CI pass. Neither PR is merged into `master`.
- Continue by committing and pushing the validated replacement to stacked PR #179, then have the user run all three tactile treatments. Select one base treatment or a bounded blend, then complete that treatment's five-view target package before F2.
