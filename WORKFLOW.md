...
- Rejected-v1 implementation commit `8820734ecc8afd1a592c9077a9e441c5c4467d5d` and its handoff commits remain as failed history in [stacked PR #179](https://github.com/dfrer/societies/pull/179), whose base is the open recovery-plan branch/PR #178 at `75b23e9de20be4797a233cb78ff8e8e1b3cd4daf`. The tactile replacement and separate local validation are pushed to that PR at implementation commit `a92e82915d636861a51d3c71ac3a7acacaa5c7a0`. GitHub reports no checks because the tests workflow is filtered to PRs based on `master` or `main`; the stacked base is excluded. This is not a CI pass. Neither PR is merged into `master`.
- Continue by having the user run all three tactile treatments. Select one base treatment or a bounded blend, then complete that treatment's five-view target package before F2.
