...
The isolated `labs/Societies.SnowGlobe` persistence experiment now has a reviewed local v5 implementation. New stores use `snow_globe_run_store/v5`; v2/v3 remain strict flat read-only inputs, while explicit v4 and v5 use the existing bounded checksum-linked framed journal. V5 adds one `PauseTransition` record/frame whose action is exactly `Pause` or `Resume`, whose tick and state/event digests bind the current deterministic world, and whose application does not change that world, its events, or its digests.

`SnowGlobePersistedSession.CreateNew(directory, identity, adapter)` starts v5 running. The four-argument create overload with `true` commits the initial Pause before returning. `Reopen(directory, identity, adapter)` derives committed v5 pause state; the four-argument reopen overload is retained only for v4's historical ephemeral pause input and rejects v5 before writer ownership. A state-changing v5 Pause/Resume commits, strictly rereads/reconstructs, and then publishes; an already-target call is applied without appending or consuming capacity. New v5 participant evaluations require durable Pause, while retries of an existing receipt remain idempotent after Resume.

Pending v5 pause frames share v4's existing one-continuation/two-segment recovery budget. A complete uncommitted pause payload is validated and abandoned at the prior durable state; partial payload/commit evidence fails closed, while a fully committed transition restores without a continuation. V4 scheduled recovery and its read-only provenance receipt remain unchanged. `SnowGlobePersistedRunInspector.Inspect` still returns an inert snapshot with `IsPaused=true` for every schema rather than exposing historical pause, and `InspectRecoveryProvenance` returns no receipt for v5.

Final evidence is 25/25 new v5 Release tests, 186/186 persistence compatibility tests, a 239/239 aggregate persistence/provider-enum selection, the full 969/969 Snow Globe Release suite, a zero-warning/zero-error Release library build, and clean diff checks. Independent migration/determinism/public-interface review is FINAL GO with no P0-P3 findings. Implementation commit `9b4b1c4` was published through [PR #135](https://github.com/dfrer/societies/pull/135); required `build-test-smoke` passed in 4m34s and the PR merged to `master` as `6505e01`. See [the v5 contract](planning/active/snow-globe-v5-durable-pause.md). This is ordinary deterministic restart evidence, not power-loss, hardware-durability, cross-host, or exactly-once certification. No `src/societies/`, provider, credential, paid, network, live-state, or `ObserverShell` behavior changed.

The current [OpenRouter contract](labs/Societies.SnowGlobe/OPENROUTER_PREMIUM_EVIDENCE.md) and [ADR 0015](docs/adr/0015-offline-openrouter-premium-evidence-boundary.md) record the production one-shot bridge, three separately consumed paid authorities, the zero-I/O readiness manifest, and the closed fourth-authorization preflight. Manifest `snow_globe_openrouter_paid_run_readiness_manifest/v1`, digest `d1e653468fd7a39e33ad355297adf96c48bde97e40a73f6b6c6812623553f737`, freezes the Luna/Azure-ZDR one-attempt contract and behaviorally probes the in-memory executor, but it did not cover fixed production state-root generation rollover.

On 2026-08-22 the exact fourth scope was consumed by one terminal `preflight_already_attempted`/exit 1 before metadata verification; no provider or paid action occurred. The completed v2 composition passed 55/55 focused, 868/868 full SnowGlobe Release, 67/67 CLI/security, zero-warning Release builds, and GO review. The later provisioning implementation gate passed 881/881 full SnowGlobe and 70/70 full CLI/security. A dedicated real Global mutex abandonment regression in `OpenRouterPremiumWindowsStateTrustAnchorTests` proves fail-closed `state_trust_anchor_provisioning_indeterminate`, releases recovered ownership, and permits later reacquisition. The later lease-zero conformance follow-up closes the former residual P3 with focused 10/10, full SnowGlobe Release 885/885, full CLI/security 70/70, and both lab and CLI Release builds at 0 warnings/0 errors; independent deep security review is GO with no P0-P3. The exact anchor-provisioning command was invoked once and exited 0; its authority is consumed. The fifth-run evidence and current action are recorded below.

## Current OpenRouter fifth paid-run evidence

The fifth authority used the exact terms up to 12 sequential requests, no retries or alternate providers, maximum aggregate `$0.018`. One session ran preflight, `record-once`, and local `validate` exactly once; authority digest `40655d2535520757b411595593deda6a714127e5366cf9afb3292aab0f3bc2d6`, generation `g2-03d00c2b41770f885e7ce27c9a4545cd9a420161b00351573fc2886b6c886df2`, manifest SHA `cd71868d349bb71b88d5b819a14e166fb520055d706f6da95025d04f33325d83`.

Evidence: 2,171 bytes/SHA `7d449d77c3b82ff1984a0e1d33c3026b80566321938418b2eb363ff1aa9f1bd8`, terminal `provider_response_rejected`, one exchange, `cq1` only, `SubmissionUnknown` / `ChargeState.Unknown`, zero trusted tokens, zero local settlement, null proposal. Receipt: 793 bytes/SHA `7f42dfd15ffe1e4134d7110a9f491f9dfaae1632d69e14180bf47dbc6056dc49`; journal: SHA `b79acbe2cff7b2a72bee53db5c3663229de035f3700c5624b5d6f2a2dcffa161`, four records, terminal checksum `e3b987863489740096370a00a3b34374e16e7ab59d817e59743ddf2833f7b5b3`. No retry or second admit/dispatch; both tombstones present. Charge is unknown, and the evidence lacks HTTP status, finish reason, response body, redirect trace, and parser-rejection detail. The authority is consumed; no stage may be rerun.
...
No retained live/provider artifact or comparison file was produced; offline tests did construct in-memory artifacts. No provider, network, live traffic, or workspace mutation occurred. Independent deep review was FINAL COMPARISON GO with no P0-P2 findings and exact bindings. The comparison action and former Design-It-Twice action are historical/superseded.
...
The bounded offline local-premium comparison is complete and historical. Its former Design-It-Twice action is historical/superseded.
...
Attempt-001 through Attempt-004 chronology and v1-v3 artifacts remain preserved; their actions are historical/superseded. Attempt-005 and the bounded offline comparison are complete; the former Design-It-Twice action is historical/superseded.

## Current Prototype

See [CURRENT_BUILD.md](CURRENT_BUILD.md) for the up-to-date prototype scope, validation commands, and implementation details.
Current implemented reality includes W2-04/W2-05, W2-06, W3-01 through W3-03 merged through PRs #122-#124, W3-04 merged through PR #171 at `ea1f7ee`, the W3-05 test/evidence slice merged through PR #173 at `f8eb222`, and the W3-05 player-selection follow-up merged through PR #175 at `36c445b`. The 2026-08-26 user-led play assessment failed substantially, and the later ER-01 HUD/interaction recovery remained unacceptable. Draft PR #177 stays unmerged. Societies is now explicitly treated as Snow Globe's embodied player-facing frontend, with the deterministic runtime retained as world authority. No gameplay implementation resumes until the [Snow Globe Frontend Product Recovery Plan](planning/active/v3-snow-globe-frontend-recovery-plan.md) passes F0 product definition and F1 visual-target acceptance.

W2-04/W2-05 remain historical merged milestones; see [CURRENT_BUILD.md](CURRENT_BUILD.md), [W3-03 validation evidence](planning/active/evidence/v3-w3-03-validation.json), [W3-04 validation evidence](planning/active/evidence/v3-w3-04-validation.json), [W3-05 validation evidence](planning/active/evidence/v3-w3-05-validation.json), and [the player-selection follow-up evidence](planning/active/evidence/v3-w3-05-player-civic-selection-validation.json). W3-04 adds a provider-neutral bounded observation/proposal contract and deterministic fallback for recording a citizen stance on an already-selected policy. `SelectCivicPolicy` remains the sole policy mutation path. PR #175 exposes `4` Protect, `5` Drawdown, and `6` for one existing `deterministic_fallback | civic.cognition.decision` result, with restored-history duplicate rejection. Automated acceptance and delivery are green, but the later manual product assessment is red. No playability, deployment, certification, or release-readiness claim is made. No live model/provider, credentials, network path, Snow Globe integration, cognition-driven policy mutation, restoration jobs, general law, markets, or broad UI were added. Git/GitHub remains authoritative for delivery/merge state.
