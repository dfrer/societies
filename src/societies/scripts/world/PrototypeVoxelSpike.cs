...
namespace Societies.Core
{
    ...
    public enum VoxelEditRejection { None, StaleRevision, OutOfBounds, InvalidActor, InvalidTick, InvalidEditKind, InvalidMaterial, CellMismatch, ImmutableBedrock, RemoveAir, PlaceNonAir, NoOp, EventCapacityReached, TickMismatch }
    ...
    public sealed class VoxelChunkGeometryProjection { public VoxelChunkGeometryProjection(VoxelChunkCoord coord, IReadOnlyList<VoxelVertex> vertices, IReadOnlyList<int> indices) { Coord = coord; Vertices = vertices; Indices = indices; } public VoxelChunkCoord Coord { get; } public IReadOnlyList<VoxelVertex> Vertices { get; } public IReadOnlyList<int> Indices { get; } public int QuadCount => Indices.Count / 6; }
    ...
    public sealed class VoxelWorldModule
    {
        ...
        public const string SnapshotSchema = "societies_voxel_world_snapshot/v1", GeneratorIdentity = "societies_voxel_generator/v1", MaterialIdentity = "air0-soil1-stone2-wood3-bedrock4";
        private readonly Dictionary<VoxelChunkCoord, byte[]> _chunks = new(); private readonly List<VoxelChangeEvent> _events = new();
        public VoxelWorldModule(int seed) { Seed = seed; foreach (VoxelChunkCoord coord in ExpectedChunkCoords()) _chunks.Add(coord, GenerateChunk(seed, coord)); }
        private VoxelWorldModule(int seed, Dictionary<VoxelChunkCoord, byte[]> chunks, long revision, long sequence, IEnumerable<VoxelChangeEvent> events) { Seed = seed; _chunks = chunks; WorldRevision = revision; EventSequence = sequence; _events.AddRange(events); }
        public int Seed { get; } public long WorldRevision { get; private set; } public long EventSequence { get; private set; } public IReadOnlyList<VoxelChangeEvent> Events => new ReadOnlyCollection<VoxelChangeEvent>(_events); public string WorldIdentity => GetWorldIdentity(Seed); public string RootHash => ComputeRootHash();
        ...
        public VoxelEditResult Execute(VoxelEditCommand command)
        {
            VoxelEditRejection rejection = Validate(command); if (rejection != VoxelEditRejection.None) return new() { Rejection = rejection, WorldRevision = WorldRevision };
        ...
        { List<VoxelChunkSnapshot> chunks = ExpectedChunkCoords().Select(c => new VoxelChunkSnapshot { X = c.X, Y = c.Y, Z = c.Z, PayloadSegments = EncodeSegments(_chunks[c]), Hash = Hash(_chunks[c]) }).ToList(); return new() { Seed = Seed, MinX = MinX, MaxXExclusive = MaxXExclusive, MinY = MinY, MaxYExclusive = MaxYExclusive, MinZ = MinZ, MaxZExclusive = MaxZExclusive, WorldRevision = WorldRevision, EventSequence = EventSequence, Events = _events.Select(value => new VoxelChangeEvent(value.Sequence, value.Tick, value.ActorId, value.Kind, value.Coord, value.Before, value.After, value.Revision)).ToList(), Chunks = chunks, WorldIdentity = WorldIdentity, RootHash = RootHash }; }
        public static VoxelWorldModule Restore(VoxelWorldSnapshot snapshot)
        {
            if (snapshot == null || snapshot.Schema != SnapshotSchema || snapshot.Generator != GeneratorIdentity || snapshot.Materials != MaterialIdentity || snapshot.MinX != MinX || snapshot.MaxXExclusive != MaxXExclusive || snapshot.MinY != MinY || snapshot.MaxYExclusive != MaxYExclusive || snapshot.MinZ != MinZ || snapshot.MaxZExclusive != MaxZExclusive || snapshot.WorldRevision < 0 || snapshot.EventSequence != snapshot.WorldRevision || snapshot.EventSequence > MaximumEventCount || snapshot.Events == null || snapshot.Events.Count != snapshot.EventSequence || snapshot.Chunks == null || snapshot.Chunks.Count != ChunkCount) throw new InvalidOperationException("Voxel snapshot metadata is invalid.");
            ...
            VoxelWorldModule candidate = new(snapshot.Seed, chunks, snapshot.WorldRevision, snapshot.EventSequence, snapshot.Events);
            VoxelWorldModule replayed = new(snapshot.Seed);
            foreach (VoxelChangeEvent value in snapshot.Events)
            {
                VoxelEditResult replay = replayed.Execute(new VoxelEditCommand { ActorId = value.ActorId, Tick = value.Tick, ExpectedWorldRevision = value.Sequence - 1, Kind = value.Kind, Coord = value.Coord, ExpectedBefore = value.Before, After = value.After });
        ...
        private VoxelEditRejection Validate(VoxelEditCommand command)
        {
            ...
            if (command.Kind == VoxelEditKind.Remove && command.After != VoxelMaterialId.Air) return VoxelEditRejection.NoOp; if (command.Kind == VoxelEditKind.Place && command.After is not (VoxelMaterialId.Soil or VoxelMaterialId.Stone or VoxelMaterialId.Wood)) return VoxelEditRejection.InvalidMaterial; if (command.Kind == VoxelEditKind.Remove && current == VoxelMaterialId.Air) return VoxelEditRejection.RemoveAir; if (command.Kind == VoxelEditKind.Place && current != VoxelMaterialId.Air) return VoxelEditRejection.PlaceNonAir; return current == command.After ? VoxelEditRejection.NoOp : VoxelEditRejection.None;
        }
        private VoxelChunkGeometryProjection BuildGeometry(VoxelChunkCoord chunk)
        {
            ...
            return new(chunk, vertices.AsReadOnly(), indices.AsReadOnly());
        }
        ...
        private static byte[] GenerateChunk(int seed, VoxelChunkCoord chunk) { byte[] cells = new byte[ChunkVoxelCount]; for (int z=0;z<ChunkDepth;z++) for(int x=0;x<ChunkWidth;x++) { int wx=chunk.X*ChunkWidth+x,wz=chunk.Z*ChunkDepth+z, terrain=8+(int)(Hash32(seed,wx,wz,1)%7); for(int y=0;y<ChunkHeight;y++) cells[(y*ChunkWidth*ChunkDepth)+(z*ChunkWidth)+x]=(byte)(y==0?VoxelMaterialId.Bedrock:y<terrain-3?VoxelMaterialId.Stone:y<=terrain?VoxelMaterialId.Soil:VoxelMaterialId.Air); if(Hash32(seed,wx,wz,2)%113==0 && terrain+2<ChunkHeight) for(int y=terrain+1;y<=terrain+2;y++) cells[(y*ChunkWidth*ChunkDepth)+(z*ChunkWidth)+x]=(byte)VoxelMaterialId.Wood; } return cells; }
        ...
        public static string GetWorldIdentity(int seed) => Hash(Encoding.UTF8.GetBytes(FormattableString.Invariant($"{GeneratorIdentity}|{MaterialIdentity}|{seed}|{MinX}:{MaxXExclusive}|{MinY}:{MaxYExclusive}|{MinZ}:{MaxZExclusive}")));
