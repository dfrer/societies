...
## Current v4 score-summary handoff (offline; record-once pending)
...
The v4 schema identities are plan `snow_globe_ollama_recording_composition_plan/v4`, receipt `snow_globe_ollama_loopback_recording_receipt/v4`, and artifact `snow_globe_ollama_recording_execution_artifact/v4`; the artifact filename/path is `artifacts/snowglobe/local-model/qwen3.5-4b-recording-execution-v4.json`. Transport remains v3 and the high-level adapter/profile remains v2. The receipt is canonical digest-only and the artifact embeds the summary once. The two-input `LocalPremiumComparison` overload emits local cognition facts only (`premium`, `premium_cost`, `performance_delta`, and `quality_delta` null; status `insufficient_live_premium_evidence`); the one-input golden is unchanged. No live v4 artifact exists.
