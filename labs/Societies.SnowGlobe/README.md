...
`Societies.SnowGlobe.BenchmarkCli` is the Windows-pinned process boundary for the frozen first compatibility cell. Fresh explicit authority was consumed by one one-shot attempt; the corrected evidence lease passed. `GET /api/tags` completed 1/1 with HTTP 200 in 538.1 us. Warmup `POST /api/generate` was attempted once but completed 0 and ended HTTP 500 after approximately 5.69 s because an external ad-hoc monitor stopped the CLI; measured requests remained 0. There was no canonical evidence JSON, model-quality/intelligence result, retry, or raw output. Maximum observed VRAM was 6,302/8,192 MiB, below the 6,963.2 MiB threshold. No actual non-loopback connection was observed; cleanup reported zero Ollama/llama-server/live rows. Root cause was wrapper misclassification of transient unconnected `Bound 0.0.0.0:61081 -> 0.0.0.0:0` as outbound. The offline fix adds a repo-owned `WindowsTcpExposureMonitor` contract, Program pre/post use, and deterministic tests: reject non-loopback LISTEN or ESTABLISHED real peers, ignore Bound wildcard-zero. The monitor strictly queries bounded AF_INET and AF_INET6 owner tables, accepts only a native 0..8 alignment tail, passes the native read-only dual-family smoke, samples concurrently across the run, and cancels on observed non-loopback LISTEN or ESTABLISHED real-peer violations. Exact transport PID/loopback checks remain; bounded samples cannot guarantee absence during unsampled transients. CLI tests pass 56/56 and the build is clean; final independent review is CODE GO with no P0-P2 findings.

There is no current live authority. After final independent review, request fresh explicit authority for a new one-shot benchmark capability; a future attempt is not a retry. The default PATH Ollama 0.18.2 remains untouched; the pinned E: runtime/model remains the only local target.
