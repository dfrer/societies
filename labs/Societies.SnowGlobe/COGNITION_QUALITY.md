...
The accepted OpenRouter artifact already retains all twelve normalized proposals and can be projected offline without another call. The accepted Ollama v5 artifact deliberately retains only the score summary, so it cannot be symmetrically re-scored or used for useful-variation evidence. A future separately governed Ollama recording must produce the additive v6 execution artifact before a final comparison; no provider, runtime, credential, network, or live-state action occurred while implementing this boundary.
