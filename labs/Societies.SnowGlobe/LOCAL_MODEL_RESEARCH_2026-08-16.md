...
The first separately authorized live compatibility candidate is **Ollama `qwen3.5:4b`**. The official listing describes a 3.4 GB, 4.66B-parameter, Q4_K_M artifact under Apache 2.0 and advertises 256K context. Those artifact facts are separate from this lab's first proposed 4K-context/96-output-token test cell. The exact installed digest, size, family, parameter count, and quantization must be locked from `/api/tags` after separate download approval; do not infer an exact `qwen3.5:4b-q4_K_M` runtime tag. Neither fit, latency, nor response quality has been measured in this lab. These are publisher/runtime claims, not acceptance results ([Ollama model page](https://ollama.com/library/qwen3.5:4b)).

The current machine is an RTX 2070 SUPER with 8,192 MiB VRAM; the supplied snapshot reports 2,784 MiB used and 5,206 MiB free, driver 581.42. Ollama 0.18.2 is installed but not running. No model weights have completed locally, and there is no llama.cpp or LM Studio install. Therefore this report authorizes no download, invocation, server start, or quality claim. The lab preflight remains: literal loopback, no credentials, redirects, retries, or proxy; one shared server; bounded bytes, depth, queue, and time; an 8,192 MiB maximum; metrics-only evidence.
