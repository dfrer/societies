...
The first separately authorized live compatibility candidate is **Ollama `qwen3.5:4b`**. A fresh committed-monitor capability completed the frozen cell: tags 1/1 HTTP 200, warmup 1/1, measured 10/10, failures=0, fallbacks=0. Outer wall time was 15.709 s; p50 887.0709 ms; p95=p99=max 1,036.6006 ms; throughput 57.637718 tok/s. Static VRAM was 6,351 MiB; sampled peak was 6,432/8,192 MiB below the 6,963.2 MiB ceiling across 179 samples. The TCP monitor recorded 482 clean bounded samples and reports `bounded_samples_do_not_guarantee_unsampled_transient_exposure`; this is not proof between samples. `external_server_startup_configuration_verified=false` remains explicit. Canonical evidence is `artifacts/snowglobe/local-model/qwen3.5-4b-frozen-benchmark-v1.json` (3,618 B, SHA-256 `961B54B7D8CFB2AEAD566579499ADB3AA21F1D85BFBE0B7C6FC504A8ADC40E0D`). No raw output was retained. Offline validation accepted the canonical artifact with 0 errors, and it was unchanged during review. Final independent evidence review is FINAL EVIDENCE GO with no P0-P2 findings. This proves local compatibility/fit/latency for this frozen cell only, not general intelligence, quality, or production readiness. These are publisher/runtime claims, not acceptance results ([Ollama model page](https://ollama.com/library/qwen3.5:4b)).
