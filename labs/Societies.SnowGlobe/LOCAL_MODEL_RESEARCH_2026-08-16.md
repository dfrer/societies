...
The first separately authorized live compatibility candidate is **Ollama `qwen3.5:4b`**. The artifact is retained and its exact identity was established by the earlier bounded smoke. A fresh benchmark capability passed the corrected evidence lease and completed `GET /api/tags` 1/1 with HTTP 200 in 538.1 us. One warmup `POST /api/generate` was attempted, completed 0, and ended HTTP 500 after approximately 5.69 s because an external ad-hoc monitor stopped the CLI; measured requests remained 0. Maximum observed VRAM was 6,302/8,192 MiB, below the 6,963.2 MiB threshold. No model load, benchmark fit, latency, response quality, or canonical evidence has been measured. The final independent monitor review is CODE GO with no P0-P2 findings; CLI validation is 56/56. The repo-owned monitor strictly queries bounded AF_INET and AF_INET6 owner tables, accepts only a native 0..8 alignment tail, passes the native read-only dual-family smoke, samples concurrently across the run, and cancels on observed non-loopback LISTEN or ESTABLISHED real-peer violations. Exact transport PID/loopback checks remain, and bounded samples cannot guarantee absence during unsampled transients. These are publisher/runtime claims, not acceptance results ([Ollama model page](https://ollama.com/library/qwen3.5:4b)).
