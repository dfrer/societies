...
# Reproduce current residual spike attribution from the clean W1-05d matrix artifacts.
./scripts/analyze-performance-spikes.ps1 -InputPath artifacts/performance/w105d-baseline-8688c27 -OutputPath artifacts/performance/w105d-baseline-8688c27/spike-analysis.json
...
- The required manifest declares 159 .NET tests: 97 fast, 11 integration, and 51 soak, plus 16 Godot headless tests.
...
- Performance result schema v5 records cold, natural-warm, and forced-invalidation cache transitions plus independent non-persistent selector and extraction-planning modes, exact-query accounting, pruning, route reuse, and phase timing. The runtime metrics CSV remains schema v4. Natural warmup advances deterministic simulation state; eager/all-pairs cache prefill remains disabled.
