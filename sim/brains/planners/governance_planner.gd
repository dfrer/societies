...
func maybe_add_goal(agent: Agent, context: PlannerContext) -> bool:
	...
	if my_faction and my_faction.founder_agent_id == agent.id:
		var claim_cost: int = int(context.tuning.get("faction_claim_cost", 15))
		if my_faction.treasury >= claim_cost * 2:
			agent.goal_stack.push_back({"type": "EXPAND_FACTION", "is_goal": true})
			return true

	if agent.has_tool("Shovel"):
		var market_pos = Vector2i(int(context.tuning.get("market_pos_x", 48)), int(context.tuning.get("market_pos_y", 48)))
		if context.state.rng.randf() < 0.05:
