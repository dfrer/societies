...
func _init_schema() -> void:
	...
	_add_schema("farm_growth_per_day", "float", false, 0.25, 0, 10, "Farm growth progress per day")
	_add_schema("farm_growth_required", "float", false, 1.0, 0, 10, "Growth required for harvest-ready")
	_add_schema("farm_yield_min", "int", false, 2, 0, 100, "Minimum farm yield per harvest")
	_add_schema("farm_yield_max", "int", false, 5, 0, 100, "Maximum farm yield per harvest")
	...
	_add_schema("tile_value_stone_weight", "float", false, 1.0, 0, 10, "Value weight for stone")
	_add_schema("tile_value_pollution_penalty", "float", false, 0.5, 0, 2, "Value penalty per pollution")
	_add_schema("homestead_claim_radius", "int", false, 3, 1, 10, "Radius for homestead claim buffer")
	_add_schema("personal_stockpile_planks", "int", false, 12, 0, 100, "Planks needed for personal stockpile")
	_add_schema("personal_stockpile_stone", "int", false, 6, 0, 100, "Stone needed for personal stockpile")
	_add_schema("personal_shelter_planks", "int", false, 20, 0, 100, "Planks needed for personal shelter")
	_add_schema("personal_shelter_stone", "int", false, 10, 0, 100, "Stone needed for personal shelter")
	...
	_add_schema("organization_farm_members_per_plot", "int", false, 6, 1, 200, "Members per farm plot target")
	_add_schema("organization_farm_food_threshold", "int", false, 25, 0, 1000, "Food stock threshold to trigger farms")
	_add_schema("organization_procure_contracts_per_day", "int", false, 2, 0, 100, "Procurement contracts per day")
	_add_schema("organization_contract_deadline_days", "int", false, 2, 1, 30, "Organization contract deadline in days")
	_add_schema("organization_contract_payout_multiplier", "float", false, 1.1, 0, 10, "Organization contract payout multiplier")
	_add_schema("organization_procure_batch_cap", "int", false, 8, 1, 100, "Max quantity per procurement contract")
	_add_schema("organization_procure_food_under", "int", false, 20, 0, 1000, "Food threshold for procurement")
	_add_schema("organization_procure_logs_under", "int", false, 12, 0, 1000, "Logs threshold for procurement")
	_add_schema("organization_procure_ore_under", "int", false, 8, 0, 1000, "Ore threshold for procurement")
	_add_schema("organization_procure_planks_under", "int", false, 10, 0, 1000, "Planks threshold for procurement")
	_add_schema("organization_procure_metal_under", "int", false, 4, 0, 1000, "Metal ingot threshold for procurement")
	_add_schema("organization_market_sell_orders_per_day", "int", false, 2, 0, 100, "Surplus sell orders per day")
	_add_schema("organization_market_sell_batch", "int", false, 10, 1, 1000, "Max quantity per org sell order")
	_add_schema("organization_market_sell_discount", "float", false, 0.0, 0, 1, "Discount applied to org sell orders")
