...
func post_contract(issuer: Agent, item: String, qty: int, payout: int, 
	...
	issuer.lock_locked_money(payout)
...
func generate_daily_contracts(state: SimState, agents: Array, market: Market, world: World, tuning: Dictionary, 
	...
	for agent in sorted_agents:
		...
		if hunger < 60 or food_count < 5:
			# Need food
			var food_roll := rng.randf()
			if food_roll < 0.6 and workshop_count > 0:
				item = "CookedMeal"
	...
	var type_roll: float = rng.randf()
			if allow_manufactured and type_roll < 0.4:
				...
				# Tools (rarely requested, usually bought directly)
				if allow_manufactured and rng.randf() < 0.3:
					if rng.randf() < 0.5: item = "Axe"
					else: item = "Pickaxe"
					qty = 1
				else:
					# Fallback to logs
					item = "Logs"
					qty = rng.randi_range(3, 5)
		
		# Ecological stress shifts choices
		if food_scarce and rng.randf() < 0.7:
			if rng.randf() < 0.5: item = "CookedMeal"; qty = rng.randi_range(1, 4)
			else: item = "Berries"; qty = rng.randi_range(5, 12)
		elif wood_scarce and rng.randf() < 0.5:
			item = "Logs"; qty = rng.randi_range(5, 10)
...
func _prune_inactive_contracts() -> void:
	...
	# Keep all active contracts, and only the most recent inactive ones
