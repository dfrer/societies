...
func tick(sim: RefCounted, state: SimState) -> void:
	var match_interval: int = state.get_tuning_int("market_match_interval_ticks", 6)
	
	if state.tick % match_interval == 0:
		var market_x: int = state.get_tuning_int("market_pos_x", 48)
		var market_y: int = state.get_tuning_int("market_pos_y", 48)
