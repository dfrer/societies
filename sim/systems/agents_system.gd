...
func tick(sim: RefCounted, state: SimState) -> void:
    var hunger_drain: float = state.tuning.get("hunger_drain_per_tick", 0.5)
    var pollution_mult: float = state.tuning.get("hunger_drain_pollution_mult", 0.5)
    for agent in state.agents:
        ...
        # Use average pollution for general hunger pressure
        var effective_drain := hunger_drain * (1.0 + avg_pollution * pollution_mult)
