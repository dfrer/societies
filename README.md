...
The v1 attempt settlement from HEAD `af0925d` is historical and superseded by v2. Local fix commit `98b3ec5` is historical. The active v2 decision is recorded in the section below: no v1 artifact/evidence was valid; both v1 authorities are consumed. The current action is one clean committed v2 preflight, then one fresh authorized v2 attempt with exactly one preflight/record/validate and no retry/alternate/fallback/download/update.

V2 code commit `a5a0823` preserves the public composition operations and CLI arguments while adding shared typed raw-free checkpoint/policy coherence and v2 plan/receipt/artifact/transport. The fixed v2 path is `artifacts/snowglobe/local-model/qwen3.5-4b-recording-execution-v2.json`; both v1 zero-byte tombstones remain untouched. Final offline validation is v2 110/110, review 129/129, SnowGlobe 615/615, RecordingCli 49/49, builds 0/0, deep CODE GO. The sole current next action is one clean committed v2 preflight, then one fresh authorized attempt with no retry/alternate/fallback/download/update.
