...
## Offline Ollama recording composition (historical first attempt; superseded by second settlement)

The current settlement is the second bounded attempt from HEAD `af0925d`: local fix commit `98b3ec5` admits the offline-proven `HttpResponseRejected` form. Exactly one preflight exited 0 with plan `ed35264777d6b6022708db092abd24e771d520b4d6b530937a72867d774decba`, exactly one record-once emitted `RECORDING_FAILED` / `composition_execution_indeterminate`, exactly one validate exited 1 `artifact_size_invalid`, and exactly one HTTP 200 POST occurred in 41.625569 s. No valid artifact/evidence exists; both authorities are consumed and no retry is allowed. The sole next action is a bounded offline diagnostic/observability decision before any separately authorized future attempt.
