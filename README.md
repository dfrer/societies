...
The current offline OpenRouter premium-evidence boundary is recorded at code commit `ef32576`; see [CURRENT_BUILD.md](CURRENT_BUILD.md), the [focused contract](labs/Societies.SnowGlobe/OPENROUTER_PREMIUM_EVIDENCE.md), and [ADR 0015](docs/adr/0015-offline-openrouter-premium-evidence-boundary.md). It is not live-ready. The one current next action is to bind an OpenRouter account identity and trusted credential lease source, approve the frozen 12,000 microusd (`$0.012`) aggregate ceiling, and then complete a security-reviewed offline preflight for dated-slug callability/current catalog+price/durable journaling before any separately authorized one-shot provider run.
