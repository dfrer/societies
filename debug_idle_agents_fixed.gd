...
func _init() -> void:
	...
	for i in range(agents.size()):
		...
		print("  Inventory:")
		var inventory_keys = agent.inventory.keys()
		for j in range(inventory_keys.size()):
			var item = inventory_keys[j]
			print("    ", item, ": ", agent.inventory[item])
		
		print("  Personality:")
		var personality_keys = agent.personality.keys()
		for j in range(personality_keys.size()):
			var trait = personality_keys[j]
