...
$runtimeFileName = "runtime-batch-metrics-v5.csv"
$requiredColumns = @(
...
foreach ($input in $InputPath) {
    ...
    if (-not $item.PSIsContainer) {
        if ($item.Name -ne $runtimeFileName) { throw "Input files must be named $runtimeFileName`: $($item.FullName)" }
        $runtimePaths.Add($item.FullName)
        continue
    }
    $direct = Join-Path $item.FullName $runtimeFileName
    if (Test-Path -LiteralPath $direct -PathType Leaf) {
        $runtimePaths.Add((Get-Item -LiteralPath $direct).FullName)
        continue
    }
    throw "No direct $runtimeFileName artifact was found under: $($item.FullName)"
}
...
foreach ($runtimePath in $runtimePaths) {
    ...
    if ($columns.Count -ne 40) { throw "Runtime CSV schema v5 must contain exactly 40 columns: $runtimePath" }
...
$output = [ordered]@{
    ...
    instrumentationContract = [ordered]@{
        ...
        runtimeMetricsFile = $runtimeFileName
