...
The script rejects a dirty source tree, explicitly rebuilds the current `HEAD` managed project in `Debug` before launch, verifies and SHA-256 hashes Godot's Debug managed assembly, and records the build command/result/configuration/hash in `reproduction.json`. It writes the five PNG files plus `capture-manifest.json` and `reproduction.json` to `docs/evidence/v3-w2-vis/<build-sha>/`, then validates the ordered five presets, scenario/seed, 10.5 terminal-trace provenance, lighting, fixed settlement animation phase, 1920x1080 viewport, per-frame tick/simulation-hour/terminal-cause/selected preset/canonical camera transforms and FOV, debug/overlay/environment-lock presentation state, truthful runtime renderer metadata, and non-empty image files. Temporary `.capture-work` compiler state is deleted on both success and failure. `-Headless` is only a transport diagnostic: its output cannot be marked reviewed visual evidence.
...
W2-04 merged through PR #117 at master `d519d4d`. A pre-existing exhaustive-reference parity leak was repaired by applying the exact-bounded shadow frontier only to backlog diagnostics; shipped/default selection, capped-backlog classification, and persisted snapshots remain unchanged. Its historical 8.0 no-input trace is tick `9777`, event `7495`, SHA-256 `ab219da9ad492a0011bd70160548d29da3420569eb6a89facd74c6c63145a880`; it is not the V3-W2-VIS capture source. The capture contract is instead pinned to the repeated 10.5 trace at tick `9777`, `Collapsed` / `IncapacitatedHold`, event `8148`, SHA-256 `69f3e22402e31a53b1d4c16899883956fcc5fdb14fbe47d8a4eb8baef007174f`. The scripted default snapshot remains SHA-256 `a81d7083649911a3244446bcde991f215dc0237ea2aa8967b94bfc0c9a98bd3b`. Full validation is green at 269/269 .NET (0 failed/skipped) and 22/22 Godot tests, and the Debug build has zero warnings. W2-VIS is nevertheless **implemented but evidence-pending** until a clean-build five-image capture and representative `empty_stores` W1-03 safety evidence are green. W2-05 is the next contracted slice after that gate; W2-06 still owns observed playtests and the Continue V3 decision. The Weeks 3-4 plan remains Draft/Conditional and inactive.
