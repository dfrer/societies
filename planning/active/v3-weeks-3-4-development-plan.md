...
W2-06 hard safety gates are green at `478a4d9`, and W3-01 through W3-05 are merged; delivery truth remains in Git/GitHub. Keep W3-06+ and broader Weeks 3-4 inactive. The local W3-05 follow-up now exposes the minimal `4`/`5` civic input and F3 interest inspection path, but its full delivery validation and human author smoke remain incomplete.
...
The historical test/evidence boundary had no player-facing policy input. The current local follow-up supplies `GameManager` key routes (`4` Protect, `5` Drawdown) that call only `PrototypeRuntimeSession.SelectCivicPolicy`, while the HUD reuses the existing policy/quota/health/consequence text and the F3 inspector exposes future-reeds and shelter-now support/opposition. The outcome contract and current evidence are [here](v3-w3-05-player-civic-selection.md) and [here](evidence/v3-w3-05-player-civic-selection-validation.json). It is not yet a final matrix, full-wrapper, human author-smoke, visual-acceptance, independent-review, or delivery claim.
