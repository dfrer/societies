...
Status: **implemented, evidence-pending on `feature/v3-w2-vis-baseline`.** The capture API fixes `empty_stores`, seed `1701`, initial simulation hour `10.5`, presentation lighting locked at `10.5`, five named camera presets, and configuration-sourced terminal tick `9777`; terminal simulation hour is deterministically derived rather than locked. It verifies the exact `Collapsed`/`IncapacitatedHold` causal state before saving the terminal frame. The capture script performs a clean-source Debug build before launch, records its command/result/managed-assembly hash and renderer identity, and validates the ordered capture manifest, canonical transforms, contribution fixture, and image files. Headless checks cover the five presets, stable representation node paths, normal-view crisis/citizen cues, and pure HUD layout bounds/non-overlap at both required resolutions. The prerequisite exhaustive-reference diagnostic repair is green at 269/269 .NET with 0 failed/skipped and 22/22 Godot tests with a Debug zero-warning build; shipped/default world semantics are unchanged and it is not new gameplay. Do not mark W2-VIS complete until clean-build screenshots and representative `empty_stores` W1-03 safety evidence are green. W2-05 remains next; Weeks 3-4 remain inactive.
