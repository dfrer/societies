...
- **V3-W1-03b cache-mode contracts in validation:** schema v3 now separates natural simulation preconditioning from cache treatment. `cold` clears only derived path entries, `natural_warm` retains the same-timeline cache, and `forced_invalidation` completes one prepared path segment on the first measured tick and records the exact new-version miss, endpoint/cell checks, cost change, and commit-to-first-correct-lookup interval. `run-performance-cache-modes.ps1` is the only cross-mode authority: it requires cold/warm configuration, environment, tick, and deterministic-hash identity plus a valid forced transition. Eager/all-pairs prefill remains disabled. The combined tree builds with zero warnings/errors; 7/7 focused probe/statistics tests pass; both PowerShell scripts parse; and failure artifacts keep all baseline/median/target claims false. Final positive cross-mode execution and clean Release evidence remain required before this slice is complete, so `cacheModeEvidence` and `releaseBaselineEvidence` remain false in repo truth.
