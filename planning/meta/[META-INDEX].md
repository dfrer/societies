# Meta Documentation Index

Cross-cutting project documentation and reference materials that span multiple sessions.

## Project Overview
- [Societies Comprehensive Breakdown](societies-comprehensive-breakdown.md) - Complete project breakdown and high-level architecture
- [Societies Meta Planning](societies-meta-planning.md) - Planning methodology and session structure documentation

## Technical Reference
- [Technical Constants](technical-constants.md) - Shared constants, values, and technical parameters
- [System Integration Map](system-integration-map.md) - Cross-system dependencies and integration points

## Quality Assurance
- [Consistency Audit Report](consistency-audit-report.md) - Documentation consistency and quality assessment
- [Documentation Completion Report](documentation-completion-report.md) - Session completion status and tracking

## Navigation Links
- [Master Research Index](../research/[MASTER-RESEARCH-INDEX].md) - All research documents by session
- [Master Session Index](../sessions/[AGENTS-READ-FIRST]-index.md) - All planning sessions overview
- [Directory Reorganization Report](../archives/REORGANIZATION-REPORT.md) - This reorganization documentation

---

*Last updated: 2026-02-01*
